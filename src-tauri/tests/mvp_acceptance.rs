...
fn dashboard_data() -> (tokens_lib::domain::UsageSummary, Vec<UsageRecord>) {
    ...
    let summary = state.repository().summary(&SummaryQuery::default()).unwrap();
    // The table asks for the same records, so every row is covered below.
    let recent = state
        .repository()
        .recent(&RecentQuery { limit: RecentQuery::MAX_LIMIT, ..RecentQuery::default() })
        .unwrap();
    (summary, recent)
}
...
fn totals_agree_with_the_records_on_screen() {
    ...
    let input = sum(recent.iter().map(|record| record.tokens.input.countable()).collect());
    assert_eq!((summary.totals.input.tokens, summary.totals.input.counted_records, summary.totals.input.unknown_records), input);

    let output = sum(recent.iter().map(|record| record.tokens.output.countable()).collect());
    assert_eq!((summary.totals.output.tokens, summary.totals.output.counted_records, summary.totals.output.unknown_records), output);

    let displayed = sum(recent.iter().map(|record| record.display_total.map(|t| t.tokens)).collect());
    assert_eq!(
        (summary.totals.display_total.tokens, summary.totals.display_total.counted_records, summary.totals.display_total.unknown_records),
        displayed
    );
}
...
fn cost_totals_agree_with_the_records_on_screen() {
    let (summary, recent) = dashboard_data();
    let CostTotal { by_currency, records_without_cost } = summary.totals.cost;

    let priced: Vec<_> = recent.iter().filter_map(|record| record.cost.amount.clone()).collect();
    ...
    for entry in &by_currency {
        let expected = priced
            ...
            .fold(None, |accumulated: Option<tokens_lib::domain::Money>, amount| match accumulated {
...
fn breakdowns_account_for_every_record() {
    ...
    let by_source: usize = summary.by_source.iter().map(|group| group.totals.record_count).sum();
    let by_model: usize = summary.by_model.iter().map(|group| group.totals.record_count).sum();
...
fn an_empty_store_reports_zero_records() {
    ...
    assert!(state.repository().recent(&RecentQuery::default()).unwrap().is_empty());
}
